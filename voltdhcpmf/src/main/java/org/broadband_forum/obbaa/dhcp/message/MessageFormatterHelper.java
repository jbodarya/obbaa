...
import org.broadband_forum.obbaa.dhcp.exception.MessageFormatterSyncException;
import org.broadband_forum.obbaa.dhcp.util.DeviceJsonUtils;
...
final class MessageFormatterHelper {
    ...
    static String getFilterElementsJsonListForGetRequest(GetRequest request, SchemaRegistry schemaRegistry, Device onuDevice,
        ...
        String deviceLeaf;
        if (request.getMessageId().equals(DhcpConstants.DEFAULT_MESSAGE_ID)) {
            deviceLeaf = DhcpConstants.DEVICE_MANAGEMENT;
            deviceSchemaRegistry = schemaRegistry;
        } else {
            deviceLeaf = DhcpConstants.ROOT;
        ...
        for (Element element : request.getFilter().getXmlFilterElements()) {
            ...
            if (request.getMessageId().equals(DhcpConstants.DEFAULT_MESSAGE_ID)) {
                ...
                filterElementJsonString = jsonObject.getJSONObject(DhcpConstants.OBBAA_NETWORK_MANAGER)
                        .getJSONObject(DhcpConstants.MANAGED_DEVICES).getJSONArray(DhcpConstants.DEVICE).getJSONObject(0)
                        .getJSONObject(DhcpConstants.DEVICE_MANAGEMENT).toString();
            }
        ...
        return "{\"" + DhcpConstants.NETWORK_MANAGER + DhcpConstants.COLON + deviceLeaf + "\""
                + DhcpConstants.COLON + "{" + String.join(",", filterElementsJsonList) + "}}";
    }
    ...
    static void validateConfig(String currentConfig, String postConfig, String onuDeviceName) throws MessageFormatterException {
        if (isInvalidConfig(currentConfig) || isInvalidConfig(postConfig)) {
            LOGGER.error(String.format("Unable to fetch pre/post backup configuration for %s request for device %s",
                    DhcpConstants.ONU_EDIT_OPERATION, onuDeviceName));
            throw new MessageFormatterException("Unable to fetch pre/post backup configuration for "
                    + DhcpConstants.ONU_EDIT_OPERATION + " request for device " + onuDeviceName);
            ...
            LOGGER.info(String.format("Config already is in sync (same config in cache) - skipping %s request for device %s",
                    DhcpConstants.ONU_EDIT_OPERATION, onuDeviceName));
            throw new MessageFormatterSyncException(DhcpConstants.EDIT_CONFIG_SYNCED);
        }
    }
