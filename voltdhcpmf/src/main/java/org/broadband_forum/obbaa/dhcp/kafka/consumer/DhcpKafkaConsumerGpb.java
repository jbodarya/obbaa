...
import org.broadband_forum.obbaa.dhcp.kafka.consumer.convert.MsgDeserializer;
 ...
 * </p>
 */
public class DhcpKafkaConsumerGpb extends AbstractDhcpKafkaConsumer<Msg> {

    private static final Logger LOGGER = Logger.getLogger(DhcpKafkaConsumerGpb.class);
    ...
    public DhcpKafkaConsumerGpb(VOLTDhcpManagement voltMgmt, Bundle bundle,
    ...
    public void subscribe() {
        ...
        do {
            if (m_callbackFunctions.isEmpty()) {
                LOGGER.trace("callback function is empty so breaking");
                continue;
            }
            try {
                final ConsumerRecords<String, Msg> records = m_consumer.poll(Duration.ofMillis(100));
