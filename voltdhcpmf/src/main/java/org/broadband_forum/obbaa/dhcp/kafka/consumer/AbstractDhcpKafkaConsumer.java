 ...
 * </p>
 */
public abstract class AbstractDhcpKafkaConsumer<T> implements DhcpKafkaConsumer<T> {

    private static final Logger LOGGER = Logger.getLogger(AbstractDhcpKafkaConsumer.class);
    ...
    AbstractDhcpKafkaConsumer(VOLTDhcpManagement voltMgmt, Bundle bundle, NetworkFunctionDao networkFunctionDao) {
        m_bundle = bundle;
    ...
    void updateSubscriberTopics(Set<KafkaTopic> kafkaTopicSet, Consumer<String, T> consumer) {
        ...
        for (KafkaTopic kafkaTopic : kafkaTopicSet) {
            if (kafkaTopic.getPurpose().equals(KafkaTopicPurpose.DHCP_NOTIFICATION.toString())
                ...
                isChanged = true;
            } else if (kafkaTopic.getPurpose().equals(KafkaTopicPurpose.DHCP_RESPONSE.toString())
                    && !m_callbackFunctions.contains(kafkaTopic.getTopicName())) {
    ...
    public void addNotificationCallback() {
        ...
        /* i don't think we require below topic subscriptions in dhcp */
    ...
    void subscribeTopics() {
        Set<String> nwFunctionNames = m_networkFunctionDao.getAllNetworkFunctionNames();
    ...
    private void registerAllNetWorkFunctionTopics(Set<String> nwFunctionNames) {
        ...
        for (String name : nwFunctionNames) {
            Set<KafkaTopic> consumerTopics = m_networkFunctionDao.getKafkaConsumerTopics(name, KafkaTopicPurpose.DHCP_RESPONSE);
